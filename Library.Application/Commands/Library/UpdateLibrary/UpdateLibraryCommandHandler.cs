...
public class UpdateLibraryCommandHandler(LibraryContext context)
    : IRequestHandler<UpdateLibraryCommand, ResultResponse<Unit>>
{
    public async Task<ResultResponse<Unit>> Handle(UpdateLibraryCommand request, CancellationToken cancellationToken)
    {
        var library =
            await context.Libraries.FirstOrDefaultAsync(l => l.Id == request.LibraryUnitId, cancellationToken);
        if (library is null)
